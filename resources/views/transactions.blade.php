    ...
    <style>
        ...
        * {
        ...
        .sidebar {
            ...
            height: 100vh;
            position: fixed;
            left: -250px; /* Adjusted to match width */
            top: 0;
        ...
        <main class="transaction-page" aria-nav-expanded="false">
            ...
            <section class="due-amount-section" aria-label="Due amount">
                <div class="due-amount-here">
                    ...
                    @if($dueAmount > 0)
                        <form action="{{ route('transaction.pay') }}" method="POST" style="display: inline-block; margin-left: 20px;">
                            @csrf
                            <select name="borrowed_book_id" required>
                                <option value="" disabled selected>Select book to pay</option>
                                @foreach($borrowedBooks->where('late_fee', '>', 0) as $book)
                                    <option value="{{ $book->id }}">
                                        {{ $book->book->title }} - Late Fee: ${{ number_format($book->late_fee, 2) }}
                                    </option>
                                @endforeach
                            </select>
                            <input type="number" name="amount" min="0.01" step="0.01" placeholder="Amount" required style="width: 100px; margin-left: 10px;">
                            <button type="submit" class="pay-button" style="background: linear-gradient(90deg, #27ae60 0%, #2ecc71 100%); cursor: pointer;">
                                Pay Now
                            </button>
                        </form>
                    @endif
                    ...
                    <article class="book-entry" style="--index: {{ $loop->index }}">
                        <div class="book-info">
                            ...
                            @if($book->late_fee > 0 && !$book->returned_at)
                                <div class="late-fee">Late Fee: ${{ number_format(max(0, $book->late_fee), 2) }}</div>
                            @endif
                            @if($book->returned_at)
                                @php
                                ...
                                @endphp
                                @if($userRating)
                                    <div class="rating-thank-you-message">Thanks for the rating!</div>
                                @else
                       ...
                       <div class="status {{ $book->returned_at ? 'returned' : ($book->due_date < now() && !$book->returned_at ? 'due' : 'borrowed') }}">
                            ...
                            @if($book->returned_at && $book->late_fee > 0 && $book->status !== 'paid' && auth()->user()->is_admin)
                                <form action="{{ route('admin.markAsPaid', $book) }}" method="POST" style="display:inline; margin-left: 10px;">
                                    @csrf
                                    <button type="submit" class="pay-button" onclick="return confirm('Are you sure you want to mark this due as paid?');">Pay Due</button>
                                </form>
                            @endif
                        </div>
                    </article>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (hoverArea && sidebar) {
                const hoverRegion = [hoverArea, sidebar];

                hoverRegion.forEach(element => {
                    ...
                    element.addEventListener('mouseleave', function() {
                        if (window.innerWidth > 768) {
                            closeTimeout = setTimeout(() => {
                                closeSidebar();
                            }, 300); // Delay to allow moving between hover-area and sidebar
                        }
                    });
                });

                // Cancel close if re-entering either element
                hoverRegion.forEach(element => {
                    element.addEventListener('mouseenter', function() {
                        if (window.innerWidth > 768 && closeTimeout) {
                            clearTimeout(closeTimeout);
                            closeTimeout = null;
                        }
                    });
                ...
                document.addEventListener('keydown', function(e) {
                    if (e.key === 'Escape' && sidebar.getAttribute('aria-expanded') === 'true' && window.innerWidth > 768) {
                        closeSidebar();
                    }
                });
            }
            ...
            ratingForms.forEach(form => {
                form.addEventListener('submit', function(event) {
                    ...
                    .then(data => {
                        ...
                        thankYouMessage.className = 'rating-thank-you-message';
                        thankYouMessage.textContent = 'Thanks for the rating!';
