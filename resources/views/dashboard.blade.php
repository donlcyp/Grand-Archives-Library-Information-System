...
<head>
    ...
    <style>
        ...
        * {
        ...
        .most-read-book-card {
            height: 320px; /* Increased height for Most Read */
        }
        ...
        .most-read-book-card img {
            height: 210px; /* Slightly reduced to give more space to book-info */
        }
        ...
        .book-card .quantity, .book-card .borrow-count {
            color: #121246;
            font-size: 0.7rem;
        ...
        @media (max-width: 768px) {
            ...
            .most-read-book-card {
                height: 290px; /* Adjusted for Most Read */
            }
        ...
        @media (max-width: 480px) {
            ...
            .most-read-book-card {
                height: 280px; /* Adjusted for Most Read */
            }
        ...
        @media (max-height: 600px) {
            ...
            .most-read-book-card {
                height: 250px; /* Adjusted for Most Read */
            }
            ...
            <div class="carousel-wrapper">
                ...
                <div class="book-container" role="region" aria-label="Most read books carousel">
                    @forelse($mostReadBooks as $book)
                        <div class="book-card most-read-book-card" role="article">
                            <form action="{{ route('favorites.toggle', $book) }}" method="POST" style="position: absolute; top: 0.5rem; right: 0.5rem; z-index: 10;">
                                @csrf
                                <button type="submit" class="bookmark-icon {{ auth()->user() && auth()->user()->favorites->contains($book->id) ? 'favorited' : '' }}" style="background: none; border: none; padding: 0; cursor: pointer;" title="Toggle favorite" aria-label="Toggle favorite for {{ $book->title }}">
                                    <i class="{{ auth()->user() && auth()->user()->favorites->contains($book->id) ? 'fa-solid fa-bookmark' : 'fa-regular fa-bookmark' }}"></i>
                            ...
                            <div class="book-info">
                                ...
                                <div style="display: flex; align-items: center; gap: 0.2rem; color: #ffca08; font-size: 0.8rem; margin-bottom: 0.15rem; justify-content: center;">
                                    @php
                                        $roundedRating = round($book->average_rating);
                                    @endphp
                                    @for ($i = 1; $i <= 5; $i++)
                                ...
                                <div class="borrow-count">Borrowed: {{ $book->borrow_count }} {{ $book->borrow_count == 1 ? 'time' : 'times' }}</div>
                                <div class="quantity">Available: {{ $book->quantity }}</div>
                                @if($book->quantity > 0)
                                    <form action="{{ route('dashboard.borrow', $book) }}" method="POST" style="display:inline;">
                                        @csrf
